...
# Install sxm-player from local source with pinned deps
COPY ./sxm-player /tmp/sxm-player
RUN pip install --no-cache-dir -r /tmp/sxm-player/requirements.txt \
    && pip install --no-cache-dir --no-deps /tmp/sxm-player \
    && rm -rf /tmp/sxm-player
